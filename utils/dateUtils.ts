...
export function getDays(days: number, selectedDate: Date) {
  ...
  if (days == 7) {
    ...
    const dayOfWeekOfFirstDayOfMonth =
      getDayOfWeekForFirstDayOfMonth(selectedDate);
    const dayOfWeekOfLastDayOfMonth =
      getDayOfWeekForLastDayOfMonth(selectedDate);
    const daysToHaveOnView =
      dayOfWeekOfFirstDayOfMonth -
      1 +
      daysInMonth +
      (7 - dayOfWeekOfLastDayOfMonth);
    const dayInMonthOffsetToDaysToHaveOnView =
      dayInMonth + (dayOfWeekOfFirstDayOfMonth - 2);
    const selectedDateTime = selectedDate.getTime();
    for (let i = 0; i < daysToHaveOnView; i++) {
      daysArray[i] = new Date(
        selectedDateTime +
          (i - dayInMonthOffsetToDaysToHaveOnView) * 24 * 60 * 60 * 1000
      );
    }
    return daysArray;
  }
...
export function incrementDate(date: Date, days: number) {
  if (days == 30) {
    ...
    const newDate = new Date(
      date.getTime() + daysUntilNextMonth * 24 * 60 * 60 * 1000
    );
...
export function decrementDate(date: Date, days: number) {
  if (days == 30) {
    return getMiddleDayOfMonth(
      new Date(date.getTime() - (date.getDate() + 10) * 24 * 60 * 60 * 1000)
    );
  } else {
    return new Date(date.getTime() - days * 24 * 60 * 60 * 1000);
  }
...
export function dateRangeTimeValid(
  datePart: string,
  timePart: string,
  datePartEnd: string,
  timePartEnd: string
  ...
  return (
    new Date(`${datePart}T${timePart}`).getTime() <=
    new Date(`${datePartEnd}T${timePartEnd}`).getTime()
  );
}

export function dateRangeValid(datePart: string, datePartEnd: string): boolean {
  return (
    new Date(`${datePart}T00:00`).getTime() <=
    new Date(`${datePartEnd}T00:00`).getTime()
  );
}
